...
val logregdataall = (
    data.select(
        ...
        $"Daily Time Spent on Site", $"Age", $"Area Income", $"Daily Internet Usage", $"Timestamp", $"Male",
...
val assembler = (
    new VectorAssembler().setInputCols(Array("Daily Time Spent on Site", "Age", "Area Income", "Daily Internet Usage", "Male", "Hour"))
...
val pipline = new Pipeline().setStages(Array(assembler, lr))
...
println(metrics.confusionMatrix) // 97% accurate
